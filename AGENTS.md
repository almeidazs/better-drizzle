...
- **JSONB path shorthand**: PostgreSQL JSONB filters accept dotted paths directly on the column (`{ metadata: { 'profile.age': { gte: 18 } } }`). The legacy `{ json: { ... } }` wrapper remains supported and is the unambiguous escape hatch for root paths and literal dotted object keys; direct shorthand intentionally requires every key to contain a dot so ordinary JSON document equality remains unchanged.
