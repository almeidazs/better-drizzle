...
- **JSONB path mutations**: update operations (`update`, `updateMany`, `updateEach`, `upsert`, `upsertMany`) accept dotted paths in `data` and compile them to chained PostgreSQL `jsonb_set(..., true)` calls with JSON-encoded bound params. One value must be all dotted keys (partial update) or no dotted keys (full replacement); mixing throws `OPERATION_ERROR`, and the `{ json: ... }` wrapper covers single-level keys plus strictly-typed paths. Values accept any JSON-encodable payload; `undefined`/unencodable throws. Only dotted/wrapper path shapes are dialect-gated (non-PostgreSQL fails fast with `JSONB_MUTATION_UNSUPPORTED` before shape validation); full-document replacements work on any dialect. A top-level `json` object key on a `PgJsonb` column is reserved for the wrapper; literal dotted keys always compile to nested paths.
