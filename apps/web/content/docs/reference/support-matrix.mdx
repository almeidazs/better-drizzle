...
Typed JSONB filters require `jsonb(...).$type<T>()` and support scalar dot paths only. Path mutations accept dotted paths in update `data` with JSON-encodable values (objects and arrays allowed); untyped columns accept untyped dotted paths plus the `{ json: ... }` wrapper. Full-document replacements are not dialect-gated.
