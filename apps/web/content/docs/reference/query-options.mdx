...
<TypeTable
	type={{
		...
		after: {
			description: 'Cursor token to read rows after (cursor mode).',
			type: 'CursorInput',
		},
		before: {
			description: 'Cursor token to read rows before (cursor mode).',
			type: 'CursorInput',
		},
	}}
/>

It returns `{ data, pagination: { count, hasNext, hasPrevious } }`. See [pagination](/docs/querying/pagination).
