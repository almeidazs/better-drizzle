...
const {
	...
	data, // inserted rows, when the driver supports RETURNING
...
```ts tab="better-drizzle" tab-group="orm"
const { count } = await client.users.updateMany({
	where: { active: true },
	data: { active: false },
}); // count: rows updated
```
...
```ts tab="better-drizzle" tab-group="orm"
const { data } = await client.users.updateEach({
	by: users.id,
	data: [
...
| `update` | `object` | Yes | Per-column callbacks `(row) => value \| SQL` that resolve the new value for each row |
...
A callback can return a Drizzle `SQL` value instead of a plain value. It is placed in that row's `CASE` branch as is, so it can call SQL functions or reference columns. Returning `undefined` throws.
...
const { count } = await client.users.updateEach({
	...
	update: {
		name: (row) => sql`upper(${row.name})`,
		city: () => sql`coalesce(${users.city}, 'Unknown')`,
	},
});
```
...
```ts tab="better-drizzle" tab-group="orm"
const { count } = await client.users.deleteMany({
	where: { active: false },
}); // count: rows deleted
```
...
const { count, data } = await client.users.upsertMany({
	data: [
...
- `target` is required: one column key (`'email'`) or an array of keys (`['tenantId', 'email']`)
- `update` can be `'all'`, a column list, an object, or a function
- `batchSize` chunks large inserts without changing the API. It must be a positive integer, otherwise the call throws `batchSize must be a positive integer.`
