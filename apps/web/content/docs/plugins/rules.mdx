...
This table shows the level each preset sets. A dash means the rule is off in that preset.
...
| `requireStableOrderByForCursor` | — | warn | error |
| `maxIncludeDepth` | — | warn, 3 | error, 2 |
| `maxIncludeRelations` | — | warn, 5 | error, 3 |
| `requireTransactionForLock` | warn | warn | error |
| `noLockWithInclude` | error | error | error |
| `noInvalidLockCombination` | error | error | error |
| `requireOrderByForSkipLocked` | — | warn | error |
| `requireLimitForSkipLocked` | error | error | error |
| `noRawUnsafe` | error | error | error |
| `requireRawComment` | — | — | warn, 8 chars |
| `requireRawTimeout` | — | warn | error |
| `noRawMutation` | — | warn | error |
| `noRawWithoutTransaction` | — | — | warn, mutations only |
| `requireTimeoutForLongRunningOperation` | — | warn, max 60s | error, max 30s |
| `requireTenantContext` | — | error | error |
| `noTenantBypassWithoutSystem` | — | — | error |
| `noTenantColumnOverride` | — | — | error |
| `requireTenantOnCreate` | — | — | error |
| `requireTenantOnUpdate` | — | — | error |
| `requireTenantOnDelete` | — | — | error |
| `noSensitiveSelect` | — | error | error |
| `requireExplicitSensitiveAccessReason` | — | — | error |
| `noHardDeleteOnSoftDeleteModel` | — | error | error |
| `requireHardDeleteReason` | — | — | error |
| `noQueryDeletedWithoutExplicitOptIn` | — | — | warn |
| `noSystemModeWithoutReason` | — | warn | error |
...
The resolved limit is `take` or `limit`. For `paginate` it also reads `perPage`. With `noUnboundedFindMany`, `allowWithLimit` accepts either `take` or `limit`, and `allowSmallStaticModels` currently behaves like `allowWithWhere`.
