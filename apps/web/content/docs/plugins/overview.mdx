...
icon: Blocks
seoTitle: 'Plugins - Extend Drizzle ORM Clients'
seoDescription: 'The better-drizzle plugin system for Drizzle ORM: setup logic, query transforms, lifecycle hooks, typed operation args, and client and model extensions.'
---
...
const client = better(db, {
	...
	plugins: [
		...
		betterZod({ validate: { create: true, update: true } }),
...
<Cards>
	<Card title="ESLint" href="/docs/plugins/eslint">
		Add static guardrails in the IDE and CI for direct Better Drizzle call
		sites, with rules and options documented on the package page.
	</Card>
	...
	<Card title="Zod" href="/docs/plugins/zod">
		Generate per-table Zod schemas, expose them on `$zod`, and validate
		payloads, query args, and results through plugin hooks.
	</Card>
...
| --- | --- | --- |
| ESLint | you want IDE and CI feedback before code runs | flags the statically-checkable subset of Better Drizzle guardrails |
...
| Zod | you want generated runtime schemas for inputs and results | builds per-table Zod schemas and validates Better Drizzle operations |
...
`better-drizzle/eslint` is intentionally separate from the runtime plugin pipeline. It mirrors the statically-checkable subset of `better-drizzle/rules` for direct call sites, while runtime-only concerns such as transaction state, merged `meta`, and dialect behavior stay in `better-drizzle/rules`.
