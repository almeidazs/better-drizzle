...
seoTitle: 'Drizzle ORM vs better-drizzle Comparison'
seoDescription: 'Compare raw Drizzle ORM queries with better-drizzle: what the repository API changes, what it leaves alone, and when to write raw Drizzle instead.'
---
...
Both are fully typed. The difference shows up when you have *dozens* of these across a codebase — the better-drizzle form is the same shape every time, and the relation join is derived from your schema instead of hand-written.
...
| Pagination | rebuild page metadata and cursor navigation each time | `paginate()` / `cursor()` → `{ data, pagination }` |
