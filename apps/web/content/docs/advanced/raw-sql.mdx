...
icon: Terminal
seoTitle: 'Raw SQL in Drizzle ORM - $raw & $executeRaw'
seoDescription: 'Run raw SQL with Drizzle sql templates: $raw for reads, $executeRaw for writes, and $rawUnsafe behind an explicit opt-in, with hooks and metadata.'
---
...
const { rows } = await db.execute<{ id: number; email: string }>(sql`
	select id, email
	from users
	where active = ${true}
	order by id asc
...
// node-postgres driver; no name, comment, or timeout options
const result = await db.execute<{ id: number; email: string }>(
	sql`select id, email from users order by id asc`,
);
