...
`attempts` is the total number of tries, so it must be at least `2` for any retry to happen (`1` or less disables retries). `on` accepts `'connectionError'`, `'deadlock'`, and `'serializationFailure'`; it defaults to all three. `delayMs` can be a fixed number or a function of the attempt (1-indexed) and defaults to `0` (retry immediately). An explicit `tx.rollback()` is never retried.
...
```ts tab="Drizzle" tab-group="orm"
// PostgreSQL; no timeout, name, comment, meta, or context equivalents
...
| `meta` | metadata merged over the client's `$withContext()` defaults for every operation, raw call, and lifecycle hook inside the transaction |
