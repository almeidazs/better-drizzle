...
<TypeTable
	type={{
		...
		compiled: { type: 'Readonly<Record<string, unknown>> | undefined', description: 'After create/update hooks only. The final atomic `SET` expressions when the write used atomic mutations.' },
