---
title: JSONB filters and mutations
description: PostgreSQL JSONB queries and partial updates through concise dot-path predicates, generated from your column's TypeScript shape.
icon: Braces
seoTitle: 'PostgreSQL JSONB Filters and Mutations'
seoDescription: 'Query and update PostgreSQL JSONB columns in Drizzle ORM with concise dot-path predicates and jsonb_set updates, while preserving the typed legacy wrapper.'
---
...
Add `.$type<T>()` to document the shape and keep the legacy wrapper's exact path and leaf validation:
...
The former `{ json: { ... } }` wrapper remains supported but is deprecated. Use it only for a root path without a dot, for an object whose literal key contains a dot, or when its stricter path and leaf validation is useful during migration; dotted shorthand would otherwise be ambiguous with JSON document equality.
...
## Partial updates with the same paths

Update operations (`update`, `updateMany`, `updateEach`, `upsert`, `upsertMany`) accept the same dotted paths in `data` and compile them to chained `jsonb_set` calls, so only the listed paths change:
...
await db
	.update(accounts)
	.set({
		settings: sql`jsonb_set(jsonb_set(${accounts.settings}, ARRAY['plan', 'tier'], ${JSON.stringify('pro')}::jsonb, true), ARRAY['plan', 'seats'], ${JSON.stringify(25)}::jsonb, true)`,
	})
	.where(eq(accounts.id, id));
```

Rules: every key inside one JSONB value must contain a dot, otherwise the value is treated as a full-document replacement. Use the `{ json: { ... } }` wrapper for single-level keys (e.g. `{ json: { nickname: 'Ada' } }`), which is also where the strictly-typed paths live. Mixing dotted and plain keys in one value throws `OPERATION_ERROR` instead of guessing, as does a `{ json: ... }` wrapper mixed with other keys or an empty `{ json: {} }` wrapper. A top-level `json` key holding a plain object on a `PgJsonb` column is reserved for the wrapper (`sql` fragments, scalars, and non-record values fall through to normal handling), and literal dotted keys (e.g. a full document `{ 'a.b': 1 }`) cannot be written as full-document replacements: every dotted key is compiled to `jsonb_set`, including keys inside the wrapper (unlike filters, the wrapper does not preserve literal dots). An empty `{}` is a full-document replacement, not a path update. Path values accept any JSON-encodable value; `undefined` and unencodable values throw `OPERATION_ERROR`. PostgreSQL only for path shapes: dotted or wrapper path attempts on other dialects fail fast with `JSONB_MUTATION_UNSUPPORTED`, while full-document replacements work on any dialect.
...
- **Scalar object paths only for filtering.** Arrays, object containment (`@>`), PostgreSQL JSONPath strings (`@?`, `@@`), and keys that contain a dot are intentionally outside the structured filter API. Mutations accept any JSON-encodable value at addressed paths, including objects and arrays.
